...
> ✅ **FIXED (PARTIAL)** in `fix/dvr-data-loss` — `StartRecording` now `LoadCatalog`s the prior catalog and `mergePriorCoverage` carries its `Hours`/`Available`/`Gaps` (and profiles only in the old catalog) into the fresh one, so `pruneOnce` sees and prunes pre-restart hours (retention is wall/size-anchored → correct regardless of origin). The new run anchors its own media origin so recent timeshift keeps working; playing back **across** a restart boundary needs a per-hour reader anchor (noted follow-up). Test `TestMergePriorCoverage`.
>
> ⚠️ **RESIDUAL — NOT YET FIXED (orphan-GC).** The coverage merge only carries forward hours that were *in a saved catalog*. It does NOT garbage-collect blob files that exist on disk but are referenced by **no** catalog — e.g. the in-progress `.open` hour at the instant of a restart, crash-dirty hours, or any blob written between two catalog saves. `retention.go pruneOnce` iterates only `cat.Profiles[].Hours` (age) and `catalogTotalSize` sums *catalog* sizes — `MaxSizeGB` measures the catalog, NOT actual disk `du`. So orphaned blobs are invisible to retention and accumulate **unboundedly across restarts**, exactly the failure this finding's title names.
>
> **Confirmed in production 2026-06-15:** the root filesystem reached 100% (DVR dir ≈ 466 GB of ~484 GB) after frequent restarts — disk `du` ≈ 94 GB/stream vs catalog tracking only ≈ 11 GB / 17 h. Recovered by purging the DVR directory. Note `retention_sec` was *already* set correctly (24 h) on the affected streams — it simply cannot see orphans; rapid redeploys (each a restart) compound the leak.
>
> **Follow-up fix (TODO):** add an orphan-GC pass on `StartRecording` (and periodically) that, per profile dir, deletes hour files/dirs not referenced by the catalog (or re-adopts them via `RepairStream`); and make `MaxSizeGB` enforcement account for on-disk usage so it can shed orphans as a backstop.
